...
class PINNModel(nn.Module):
    ...
    def predict_with_uncertainty(self, X_df_original_scale: pd.DataFrame, input_columns: list[str], num_samples=30, dropout_rate=0.3):
        ...
        with torch.no_grad():
            predictions = [self(X_tensor).numpy() for _ in range(num_samples)]

        predictions = np.array(predictions)
        mean_predictions = predictions.mean(axis=0)
        std_dev = predictions.std(axis=0)

        mean_predictions_original_scale = self.scaler_y.inverse_transform(mean_predictions)

        # This is a simplification. A more robust way would be to scale the std dev as well.
        std_dev_original_scale = std_dev * self.scaler_y.scale_

        return mean_predictions_original_scale, std_dev_original_scale
